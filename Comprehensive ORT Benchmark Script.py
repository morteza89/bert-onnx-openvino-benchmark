...
def warmup_ort_session(ort_session, input_dict, warmup_iterations=30):
    ...
    logger.info(f"Warming up ONNX Runtime session with {warmup_iterations} iterations...")
...
def main():
    ...
    ort_cold_times, ov_cold_times = benchmark_cold_start(ort_session, compiled_model, tokenizer, num_trials=10)
    ...
    ort_stats = benchmark_ort_with_statistics(ort_session, input_dict, iterations=2000, model_name="ONNX Runtime")
    ...
    ort_throughput, ort_count, ort_time, ort_throughput_times = benchmark_ort_throughput(ort_session, input_dict, duration_seconds=60, model_name="ONNX Runtime")
    ...
    logger.info(f"ONNX Runtime Throughput: {ort_throughput:.2f} inferences/sec ({ort_count} inferences in {ort_time:.2f}s)")
    ...
    ort_variety_stats, ov_variety_stats = benchmark_text_variety(ort_session, compiled_model, tokenizer, num_samples=500)
    
    logger.info(f"Text Variety - ONNX Runtime: {ort_variety_stats['mean']:.3f}ms ± {ort_variety_stats['std']:.3f}")
    logger.info(f"Text Variety - OpenVINO: {ov_variety_stats['mean']:.3f}ms ± {ov_variety_stats['std']:.3f}")
    
    # --- Final Summary ---
    logger.info("=== FINAL PERFORMANCE SUMMARY ===")
    logger.info(f"Standard Latency Benchmark:")
    logger.info(f"  ONNX Runtime - Mean: {ort_stats['mean']:.3f}ms, Median: {ort_stats['median']:.3f}ms, P95: {ort_stats['p95']:.3f}ms")
    ...
    logger.info(f"  ONNX Runtime - Mean: {ort_variety_stats['mean']:.3f}ms, P95: {ort_variety_stats['p95']:.3f}ms")
    ...
    logger.info(f"  ONNX Runtime - Mean: {statistics.mean(ort_cold_times):.3f}ms")
